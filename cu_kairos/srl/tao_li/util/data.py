...
class Data:
    def __init__(
        self, opt, data_file, res_files=None, triple_mode=False, preload_block_size=1000
    ):
        self.opt = opt
    ...
    def __preload(self, batches):
        ...
        self.batch_map = {}
        for i in tqdm(
            range(len(batches)), desc="preloading {0} batches".format(len(batches))
        ):
            b = batches[i]
            ...
            tok_idx_i = torch.from_numpy(
                self.f["tok_idx"][start:end, 0:seq_l_i].astype(np.int32)
            )
            sub2tok_idx_i = torch.from_numpy(
                self.f["sub2tok_idx"][start:end, 0:seq_l_i].astype(np.int32)
            )
            ...
            roleset_id_i = torch.from_numpy(
                self.f["roleset_id"][start:end].astype(np.int32)
            )
            roleset_suffix_i = torch.from_numpy(
                self.f["roleset_suffix"][start:end].astype(np.int32)
            )
            orig_seq_l_i = torch.from_numpy(
                self.f["orig_seq_l"][start:end].astype(np.int32)
            )
            srl_label_i = torch.from_numpy(
                self.f["srl_label"][
                    start:end, 0 : v_l_i.max(), 0 : orig_seq_l_i.max()
                ].astype(np.int32)
            )
            frame_idx_i = torch.from_numpy(
                self.f["frame_idx"][start:end, 0 : v_l_i.max()]
            )
            if "wp_idx" in self.f:
                wp_idx_i = torch.from_numpy(
                    self.f["wp_idx"][start:end, :].astype(np.int32)
                )
            else:
                wp_idx_i = None

            if "wpa_idx" in self.f:
                wpa_idx_i = torch.from_numpy(
                    self.f["wpa_idx"][start:end, :].astype(np.int32)
                )
            else:
                wpa_idx_i = None

            if "vn_label" in self.f:
                vn_label_i = torch.from_numpy(
                    self.f["vn_label"][
                        start:end, 0 : v_l_i.max(), 0 : orig_seq_l_i.max()
                    ].astype(np.int32)
                )
            ...
            if "vn_class" in self.f:
                vn_class_i = torch.from_numpy(
                    self.f["vn_class"][start:end, 0 : v_l_i.max()].astype(np.int32)
                )
            else:
                vn_class_i = None

            if "semlink_l" in self.f:
                semlink_l_i = torch.from_numpy(
                    self.f["semlink_l"][start:end, 0 : v_l_i.max()].astype(np.int32)
                )
                semlink_i = torch.from_numpy(
                    self.f["semlink"][start:end, 0 : v_l_i.max(), :, :].astype(np.int32)
                )
            else:
                semlink_l_i = 0
                semlink_i = None
