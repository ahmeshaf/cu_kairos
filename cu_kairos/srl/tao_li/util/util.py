...
def complete_opt(opt):
    ...
    if hasattr(opt, "roleset_suffix_dict"):
        opt.roleset_suffix, opt.roleset_suffix_map_inv = load_label_dict(
            opt.roleset_suffix_dict
        )
    ...
    if hasattr(opt, "vn_labels") and hasattr(opt, "srl_labels"):
        ...
        opt.cross2condensed = [
            [-1 for _ in opt.srl_labels] for _ in opt.vn_labels
        ]  # a matrix of (num_vn_label, num_srl_label) with indices to condensed labels
        opt.condensed2cross = []
        ...
        for i, v in enumerate(opt.vn_labels):
            ...
            for k, s in enumerate(opt.srl_labels):
                ...
                if (
                    ...
                    and s_prefix is not None
                    and v_prefix != s_prefix
                ):
                    continue
...
def is_v(l):
    return l.startswith("B-") and (
        l.split("-")[1][0].isnumeric() or l.split("-")[1] == "V"
    )
...
def max_with_mask(v, dim):
    ...
    return (
        max_v,
        max_idx,
        torch.zeros(v.shape).to(v).scatter(dim, max_idx.unsqueeze(dim), 1.0),
    )
...
def min_with_mask(v, dim):
    ...
    return (
        min_v,
        min_idx,
        torch.zeros(v.shape).to(v).scatter(dim, min_idx.unsqueeze(dim), 1.0),
    )
...
def random_duplicate(mat):
    ...
    sampling_weight = (
        mat_legit > 0
    ).float()  # sampling probabolity. If an element is nonzero then it has sampling weight of 1 otherwise 0.
    ...
    mat_legit[torch.arange(mat.size()[0]), tgt_idx] = mat_legit[
        torch.arange(mat.size()[0]), src_idx
    ]
