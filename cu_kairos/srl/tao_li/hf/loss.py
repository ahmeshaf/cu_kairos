...
class CrossCRFLoss(torch.nn.Module):
    def __init__(self, opt, shared):
        ...
        self.cross2condensed = nn.Parameter(
            torch.LongTensor(self.opt.cross2condensed), requires_grad=False
        )
        self.condensed2cross = nn.Parameter(
            torch.LongTensor(self.opt.condensed2cross), requires_grad=False
        )
        self.condensed2srl = nn.Parameter(
            torch.LongTensor(self.opt.condensed2srl), requires_grad=False
        )
        ...
        self.condensed_b_mask = [
            1 if l.startswith("B-") else 0 for l in self.opt.condensed_labels
        ]
        ...
        self.condensed_core_masks = [
            [0 for _ in self.opt.condensed_labels] for _ in range(6)
        ]  # ARG0-ARG5
        for i in range(1, len(self.opt.vn_labels)):
            for k in range(1, len(self.opt.srl_labels)):
                ...
                if (
                    p == -1
                    or self.opt.vn_labels[i].split("-")[-1] == "V"
                    or self.opt.srl_labels[k].split("-")[-1] == "V"
                ):
                    continue
        ...
        self.condensed_content_mask = nn.Parameter(
            torch.tensor(self.condensed_content_mask), requires_grad=False
        )
        self.condensed_core_masks = nn.Parameter(
            torch.tensor(self.condensed_core_masks), requires_grad=False
        )

        self.srl_core_masks = [
            [0 for _ in self.opt.srl_labels] for _ in range(6)
        ]  # ARG0-ARG5
        ...
        self.srl_core_masks = nn.Parameter(
            torch.tensor(self.srl_core_masks), requires_grad=False
        )
        print(
            "srl_core_labels", [self.opt.srl_labels[k] for k in self.srl_core_label_idx]
        )

        # (num_vn_label, num_condensed_label)
        self.vn2condensed_mask = [
            [0 for _ in self.opt.condensed_labels] for _ in self.opt.vn_labels
        ]
        ...
        self.vn2condensed_mask = nn.Parameter(
            torch.tensor(self.vn2condensed_mask, dtype=torch.bool), requires_grad=False
        )
        ...
        self.srl2condensed_mask = [
            [0 for _ in self.opt.condensed_labels] for _ in self.opt.srl_labels
        ]
        for k in range(len(self.opt.srl_labels)):
            for i in range(len(self.opt.vn_labels)):
                p = self.cross2condensed[i][k]
        ...
        self.srl2condensed_mask = nn.Parameter(
            torch.tensor(self.srl2condensed_mask, dtype=torch.bool), requires_grad=False
        )
        ...
        self.crf = ConditionalRandomField(
            num_tags=len(self.condensed_labels), constraints=constraints
        )
    ...
    def get_semlink_penalty_mask(self, a_semlink):
        ...
        # (num_v, condensed_l)
        inner_mask = inner_mask.logical_and(
            self.condensed_content_mask.view(1, 1, condensed_l).to(a_semlink.device)
        )
        ...
        disable_mask = inner_mask.logical_not().logical_and(
            self.condensed_content_mask.view(1, condensed_l).to(a_semlink.device)
        )
    ...
    def forward(self, pack):
        ...
        pretty_vn_pred, _ = self.record_log(
            "vn", self.vn_labels, flat_vn_pred, bv_idx=self.opt.vn_labels.index("B-V")
        )
        pretty_srl_pred, _ = self.record_log(
            "srl",
            self.srl_labels,
            flat_srl_pred,
            bv_idx=self.opt.srl_labels.index("B-V"),
        )

        return {
            # 'flat_vn_pred': flat_vn_pred, 'flat_srl_pred': flat_srl_pred,
            "vn_output": pretty_vn_pred,
            "srl_output": pretty_srl_pred,
        }
    ...
    def record_log(self, log_name, label_names, flat_pred_idx, bv_idx):
        ...
        for i in range(batch_l):
            ...
            pretty_role_log.append(
                self.compose_log(label_names, orig_toks[1:-1], a_pred_i[1:-1, 1:-1])
            )
    ...
    def compose_log(self, label_names, orig_toks, role_labels):
        ...
        for i, row in enumerate(role_labels):
            ...
            roles = roles + [
                "O"
            ]  # TODO, the convert_role_labels prefers the last label to be O, so bit hacky here
            if is_v(
                roles[i]
            ):  # on the diagonal, if the i-th label is a predicate, then tag it
