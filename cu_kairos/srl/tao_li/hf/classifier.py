...
class MergeConcatClassifier(torch.nn.Module):
    def __init__(self, opt, shared):
        ...
        self.vn_f_v = nn.Sequential(nn.Linear(opt.hidden_size, opt.hidden_size))

        self.vn_f_a = nn.Sequential(nn.Linear(opt.hidden_size, opt.hidden_size))
        ...
        self.srl_f_v = nn.Sequential(nn.Linear(opt.hidden_size, opt.hidden_size))

        self.srl_f_a = nn.Sequential(nn.Linear(opt.hidden_size, opt.hidden_size))
        ...
        self.v_layer = nn.Sequential(
            nn.Dropout(opt.dropout), nn.Linear(opt.hidden_size, opt.num_vn_label)
        )
    ...
    def extract_v_enc(self, enc):
        ...
        for i in range(batch_l):
            ...
            start += v_l[i]
            batch_v_idx.extend(
                ((p, q) for p, q in zip([i] * v_l[i], v_label[i, : v_l[i]]))
            )
    ...
    def forward(self, enc):
        ...
        # v predictor
        v_score = self.v_layer(vn_enc.view(-1, hidden_size)).view(
            batch_l, source_l, self.opt.num_vn_label
        )
        ...
        va_enc = torch.cat(
            [
                ...
                a_enc.expand(num_v, source_l, self.opt.hidden_size),
            ],
            dim=-1,
        )
        ...
        concated = torch.concat(
            [vn_final, srl_final], dim=-1
        )  # (num_v, source_l, hidden_size*2)
        vn_srl_score = self.vn_srl_label_layer(
            concated.view(-1, self.opt.hidden_size * 2)
        ).view(num_v, source_l, self.num_condensed_label)
        log_vn_srl = nn.LogSoftmax(-1)(vn_srl_score)

        # get vn and srl label scores as well, by merging scores from the condensed list
        cross2condensed_idx = self.cross2condensed.view(1, -1).expand(
            num_v * source_l, -1
        )  # (num_v * source_l, num_vn * num_srl)
        condensed_score = vn_srl_score.view(
            num_v * source_l, -1, 1
        )  # (num_v * source_l, num_condensed, 1)	last 1 so we can use batch_index1_select
        cross_score = batch_index1_select(
            condensed_score, cross2condensed_idx, nul_idx=-1
        ).view(num_v, source_l, self.opt.num_vn_label, self.opt.num_srl_label)
        ...
        pack = {
            "log_score": log_vn_srl,
            "label_score": vn_srl_score,
            "log_srl": log_srl,
            "srl_score": srl_score,
            "log_vn": log_vn,
            "vn_score": vn_score,
