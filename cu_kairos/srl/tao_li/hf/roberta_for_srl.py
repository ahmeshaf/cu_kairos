...
def process_options_(config):
    if hasattr(config, "arg_role_map_inv"):
        config.arg_role_map_inv = {
            int(k): v for k, v in config.arg_role_map_inv.items()
        }
    if hasattr(config, "condensed_map_inv"):
        config.condensed_map_inv = {
            int(k): v for k, v in config.condensed_map_inv.items()
        }
    ...
    if hasattr(config, "vn_class_map_inv"):
        config.vn_class_map_inv = {
            int(k): v for k, v in config.vn_class_map_inv.items()
        }
    if hasattr(config, "vn_label_map_inv"):
        config.vn_label_map_inv = {
            int(k): v for k, v in config.vn_label_map_inv.items()
        }
...
def preprocess_input(config, tokenizer, word_toks, v_idx, vnclasses, senses):
    ...
    for l in tok_l:
        sub2tok_idx.append(
            pad([p for p in range(acc, acc + l)], config.max_num_subtok, -1)
        )
        assert len(sub2tok_idx[-1]) <= config.max_num_subtok
        acc += l
    sub2tok_idx = pad(
        sub2tok_idx, len(tok_idx), [-1 for _ in range(config.max_num_subtok)]
    )
    ...
    roleset_suffix = [
        config.roleset_suffix.index(p) if p in config.roleset_suffix else 0
        for p in suffixes
    ]
    vn_class = [
        config.vn_classes.index(p) if p in config.vn_classes else 0 for p in vnclasses
    ]
